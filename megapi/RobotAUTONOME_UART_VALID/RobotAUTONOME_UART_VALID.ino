...
// pour que le robot se déplace dans la bonne direction il faut tourner le robot tel que : le robot soit tourné ds au mur du fond de la salle train (tête vers les ordinateurs si le robot est sur la maquette)
// deplus un probleme de dérive vers la droite du robot à été observé un second programe (non fonctionnel) qui règle le problème de dérivation à été fait mais l'angle ne se setup pas à 0 degrés 
...
#define UART_START_BYTE 0x24  // Le symbole '$' (Doit être PAREIL sur le RPi)
...
void UpdateSensors(){
  ...
  Wire.write(0x16); // registre Gyro Z high byte
  Wire.endTransmission();

  Wire.requestFrom(CMPS12_ADDRESS, 2);
   int16_t gz = (Wire.read() << 8) | Wire.read(); // valeur brute
  float gyroZ_dps = gz / 16.0;
  // Intégration pour obtenir l’angle
  unsigned long now = millis();
  float dt = (now - lastTime) / 1000.0; // secondes
  lastTime = now;

  angle += gyroZ_dps * dt;  // cumul de l’angle
...
void UpdateControl()
{
  ...
  float x_ref = pos_data.x;
  float y_ref = pos_data.y;
  ...
  float STOP_RADIUS_MM = 50.0f; // Gardons cette valeur pour l'instant
  ...
  if (gps_update_counter >= UPDATE_INTERVAL_CYCLES) {
      // Applique la correction. Le robot continue de s'orienter avec le modèle
      // mais sa position estimée (x, y) est forcée à la valeur GPS/RPi.
       
      // Assurez-vous d'utiliser le cast float, comme en setup()
      x = (float)pos_data.x;
      y = (float)pos_data.y;
        
      // Optionnel : Réinitialiser la distance parcourue des encodeurs
      // pour réduire le saut si la correction est importante (mais souvent non nécessaire ici)
      // Encoder_1.setCurPos(t_k_1); 
      // Encoder_2.setCurPos(t_k_2); 
