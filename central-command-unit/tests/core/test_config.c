...
TEST_REGISTER(test_config_success, "Test d'un fichier de configuration valide et complet") {
    ...
    TEST_ASSERT(strcmp(common_cfg.network.serviceIp, "127.0.0.1") == 0, "serviceIp incorrect");
    TEST_ASSERT(common_cfg.network.servicePort == 8080, "servicePort incorrect");
    TEST_ASSERT(strcmp(common_cfg.network.brokerIp, "192.168.1.100") == 0, "brokerIp incorrect");
    TEST_ASSERT(common_cfg.network.brokerPort == 5672, "brokerPort incorrect");

    // Vérification de la section [Logging]
    TEST_ASSERT(common_cfg.logging.logLevel == LOG_LEVEL_DEBUG, "logLevel incorrect");
...
TEST_REGISTER(test_config_invalid_value, "Test avec une valeur invalide pour logLevel") {
	logger_init(LOG_LEVEL_DEBUG, log_callback);
    ...
    config_common_t common_cfg;
    int result = parse_config_file(filename, &common_cfg, NULL, NULL);
    TEST_ASSERT(result != 0, "Le parsing doit échouer pour une valeur de logLevel invalide");
